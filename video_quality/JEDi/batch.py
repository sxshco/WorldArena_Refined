...
def intersect_pairs(real_dir: Path, gen_dir: Path):
    ...
    # Prefer exact stem intersection first.
    common = sorted(set(real_map.keys()) & set(gen_map.keys()))

    # Fallback: match by trailing episode token, e.g.:
    # real: episode0 <-> gen: task_episode0
    if len(common) == 0:
        def _episode_key(stem: str):
            m = re.search(r"(episode[_-]?\d+)$", stem)
            if m is None:
                return None
            return m.group(1).replace("_", "-")

        def _build_unique_key_map(stem_map):
            grouped = {}
            for stem, path in stem_map.items():
                k = _episode_key(stem)
                if k is None:
                    continue
                grouped.setdefault(k, []).append((stem, path))

            unique = {}
            for k, items in grouped.items():
                # Skip ambiguous keys to avoid accidental wrong pairing.
                if len(items) == 1:
                    unique[k] = items[0]
                else:
                    stems = [it[0] for it in items]
                    print(f"[WARN] ambiguous episode key '{k}', skip candidates: {stems}")
            return unique

        real_by_key = _build_unique_key_map(real_map)
        gen_by_key = _build_unique_key_map(gen_map)
        key_common = sorted(set(real_by_key.keys()) & set(gen_by_key.keys()))

        if len(key_common) > 0:
            print(f"[INFO] exact stem intersection is empty, fallback to episode-key matching: {len(key_common)} pairs")
            real_paths = []
            gen_paths = []
            names = []
            for k in key_common:
                real_stem, real_path = real_by_key[k]
                gen_stem, gen_path = gen_by_key[k]
                if is_valid_mp4(real_path) and is_valid_mp4(gen_path):
                    # Keep the real stem as canonical sample name for reproducibility.
                    names.append(real_stem)
                    real_paths.append(real_path)
                    gen_paths.append(gen_path)
                else:
                    print(f"[WARN] skip invalid pair by episode key '{k}': {real_stem} <-> {gen_stem}")
            return names, real_paths, gen_paths
...
def main():
    ...
    if not Path(args.model_dir).is_absolute():
        args.model_dir = str((script_dir / args.model_dir).resolve())
    ...
    jedi = JEDiMetric(model_dir=args.model_dir, config_path=args.config_path)
