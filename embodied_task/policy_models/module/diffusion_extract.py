...
class Diffusion_feature_extractor(nn.Module):
    def __init__(
        ...
        default_text_encoder = "openai/clip-vit-base-patch32"
        self.tokenizer = tokenizer if tokenizer is not None else AutoTokenizer.from_pretrained(default_text_encoder, use_fast=False)
        self.text_encoder = text_encoder if text_encoder is not None else CLIPTextModelWithProjection.from_pretrained(default_text_encoder)
        self.num_frames = 16
