...
def parse_args():
    ...
    parser.add_argument("--output_base", type=str, default="./data_action_following", help="Output base directory")
...
def _candidate_names(item: dict, task_id: str, episode_id: str, gt_video_path: Path, suffix: str = ""):
    explicit_name = item.get("generated_video_name") or item.get("generated_video")
    names = []
    if explicit_name and suffix == "":
        names.append(str(explicit_name))
    names.extend([
    ...
    if suffix:
        ...
        for name in base_names:
            stem = Path(name).stem
            names.append(f"{stem}{suffix}.mp4")
    return names


def _resolve_generated_video(gen_video_dir: Path, candidate_names):
    seen = set()
...
def process_item(item: dict, gen_video_dir: Path, output_base: Path):
    ...
    gen_root = output_base / "generated_dataset" / task_id / episode_id
    variants = {
        "1": (gen_video_dir, _candidate_names(item, task_id, episode_id, gt_video_path, suffix="")),
        "2": (gen_video_dir.parent / gen_video_dir.name.replace("_test", "_test_1"), _candidate_names(item, task_id, episode_id, gt_video_path, suffix="_1")),
        "3": (gen_video_dir.parent / gen_video_dir.name.replace("_test", "_test_2"), _candidate_names(item, task_id, episode_id, gt_video_path, suffix="_2")),
    }

    resolved = {}
    for gid, (variant_dir, candidate_names) in variants.items():
        video_dir_gen = gen_root / gid / "video"
        gen_video_path = _resolve_generated_video(variant_dir, candidate_names)
        extract_frames(gen_video_path, video_dir_gen)
        resolved[gid] = str(gen_video_path)

    return {
        "task_id": task_id,
        "episode_id": episode_id,
        "gt_video": str(gt_video_path),
        "generated_videos": resolved,
    }
